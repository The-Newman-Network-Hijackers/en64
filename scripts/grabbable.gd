...
func interact(player : Player) -> void:
	...
	# Otherwise, make a funny little sound and animation
	var tw := create_tween()
	tw.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_BACK).set_parallel()
	tw.tween_property(equipment_rect[0], "scale", Vector3.ONE, 0.5).from(Vector3(0.4, 1.8, 0.4))
	tw.tween_property(equipment_rect[1], "scale", Vector3.ONE, 0.5).from(Vector3(1.8, 0.4, 1.8))
	tw.play()
	sfx_deny.play()
