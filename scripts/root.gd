...
## Whether or not we're in "demo mode"
@onready var dmode = ProjectSettings.get_setting("application/config/demo_mode")
...
func _notification(what) -> void:
	match what:
		NOTIFICATION_CRASH:
			# Save data very quickly
			PlayerDataManager.save_data(PlayerDataManager.load_data())
