...
func _physics_process(_delta: float) -> void:
	...
	var not_invisible = Engine.get_physics_frames() % 10 < 5
...
func take_damage(damage : int, origin : Vector3 = Vector3.ZERO, knockback : float = 20.0) -> void:
	...
	set_invulnerable(true, inv_time if health > 0 else -1.0)
...
## Makes hitbox invincible
func set_invulnerable(mode : bool, time : float = -1.0) -> void:
	# Set invulnerability
	is_invulnerable = mode
	...
	# Create timer
	invulnerable_timer = get_tree().create_timer(time, false, true)
