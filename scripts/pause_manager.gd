...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event.is_action_pressed("pause"):
		...
		# Stop if we're in submenu
		if is_paused && %root_menu.process_mode == PROCESS_MODE_DISABLED:
			return
...
func _leave_pressed():
	...
	var mode = root.dmode
	lm.change_level("res://scenes/menu/demo_menu.tscn" if mode else "res://scenes/menu/main_menu.tscn")
